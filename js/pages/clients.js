/* ============================================
   Portfolio BCHS — Client Management Page (Page 4)
...
const ClientsPage = {
  ...
  renderList() {
    ...
    let html = '';
    if (active.length)  html += this.renderGroup('✅ Активные',       active);
    if (paused.length)  html += this.renderGroup('⏸ На паузе',       paused);
    if (selfmgd.length) html += this.renderGroup('🔧 Самоуправление', selfmgd);

    container.innerHTML = html;
    this.bindListEvents();
  },

  renderGroup(title, clients) {
    return `
  ...
  renderClientCard(c) {
    ...
    return `
      <div class="client-card${isHighlighted ? ' expanded' : ''}" data-id="${c.id}"
  ...
  bindListEvents() {
    document.querySelectorAll('[data-action="detail"]').forEach(btn => {
      btn.addEventListener('click', () => App.navigate('detail', btn.dataset.id));
    });
  ...
  showForm(client) {
    ...
    const bcgOpts = Object.entries(BCG_CATEGORIES).map(([k, v]) =>
      `<option value="${k}" ${client && client.bcg_category === k ? 'selected' : ''}>${v.label}</option>`
    ).join('');

    const priorityOpts = PRIORITIES.map(p =>
      `<option value="${p}" ${client && client.key_account_priority === p ? 'selected' : ''}>${p}</option>`
    ).join('');

    const statusOpts = STATUSES.map(s =>
      `<option value="${s}" ${client && client.status === s ? 'selected' : (s === 'Active' && !client ? 'selected' : '')}>${s}</option>`
    ).join('');

    container.innerHTML = `
      <hr class="divider" />
      <div class="form-section">
        <div class="form-section-title">${isEdit ? `✎ Редактировать: ${client.name}` : '+ Новый клиент'}</div>

        <div class="form-grid">
          <div class="form-group">
            <label class="form-label">Название клиента *</label>
            <input type="text" class="form-input" id="cf-name" value="${client ? this.esc(client.name) : ''}" placeholder="ООО Пример" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-input" id="cf-owner" value="${client ? this.esc(client.sales_owner || '') : ''}" placeholder="Имя менеджера" />
          </div>
        </div>

        <div class="form-group full" style="margin-top:14px">
          <label class="form-label">Стратегические заметки</label>
          <textarea class="form-textarea" id="cf-notes" rows="4">${client ? this.esc(client.strategy_notes || '') : ''}</textarea>
        </div>

        <!-- BCG Quick Reference -->
        <div style="margin-top:16px;padding:12px;background:var(--bg);border-radius:var(--radius-sm);border:1px solid var(--border-light)">
          <div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">📋 Справка: BCG × Приоритет</div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px;font-size:11.5px;color:var(--text-secondary)">
        ...
        <div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap">
          <button class="btn btn-primary" id="cf-save">${isEdit ? '💾 Сохранить изменения' : '✚ Создать клиента'}</button>
          <button class="btn btn-secondary" id="cf-cancel">Отмена</button>
        </div>
      </div>
    `;

    document.getElementById('cf-save').addEventListener('click', () => this.saveForm());
  ...
  async saveForm() {
    ...
    if (!name) {
      App.toast('Введите название клиента', 'error');
      return;
    }
    const data = {
      name,
    ...
    try {
      if (this.editingId) {
        ...
        App.toast('✅ Клиент создан', 'success');
      }
      this.editingId   = null;
      this.highlightId = null;
      const container = document.getElementById('client-form-container');
      ...
      btn.textContent = this.editingId ? '💾 Сохранить изменения' : '✚ Создать клиента';
      btn.disabled = false;
    }
  },
