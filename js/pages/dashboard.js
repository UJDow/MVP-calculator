...
const DashboardPage = {
  ...
  async render() {
    ...
    main.innerHTML = `
      <div class="page-header">
        ...
        <div class="page-subtitle">Сводка по всем клиентам · <span id="dash-updated">загрузка...</span></div>
      </div>
      <div class="filters-bar">
        <input type="text" class="search-input" id="dash-search" placeholder="🔍 Поиск по клиенту..." />
  ...
  bindFilters() {
    ...
    document.getElementById('filter-bcg').addEventListener('change', e => {
  ...
  renderList() {
    ...
    if (rows.length === 0) {
      ...
      if (alert.length > 0) html += this.renderSection('🔴 Требуют внимания', alert, 'section-alert');
      if (work.length > 0)  html += this.renderSection('🟡 В работе',         work,  'section-work');
      if (auto.length > 0)  html += this.renderSection('⚪ Автопилот / Пауза', auto,  'section-auto');
    }
    ...
    return `
      <div class="client-row${expanded ? ' expanded' : ''}" data-id="${c.id}">
        ...
        <div class="row-metrics">
          ...
          <span class="metric-chip text-muted" title="% от идеала">${potentialStr !== '—' ? potentialStr + ' ид.' : '—'}</span>
        </div>
      </div>
  ...
  renderExpanded(row) {
    ...
    // Лояльность по месяцам для мини-таблицы
    ...
    return `
      <div class="row-detail" data-detail-id="${c.id}">
        <div class="row-detail-grid">
          ...
          <div class="detail-stat">
            <span class="detail-stat-label">PC · Нагрузка (май)</span>
          ...
          <div class="detail-stat">
            ...
            <span class="detail-stat-value ${row.trend ? row.trend.cls : ''}">${row.trend ? row.trend.label : '—'}</span>
        ...
        <div class="detail-actions">
          <button class="btn btn-primary btn-sm" data-action="go-detail" data-id="${c.id}">
            📊 Карточка
          </button>
          <button class="btn btn-secondary btn-sm" data-action="go-entry" data-id="${c.id}">
            ✎ Данные
          </button>
          <button class="btn btn-secondary btn-sm" data-action="open-status" data-id="${c.id}" data-name="${c.name}">
            📝 Статус
          </button>
        </div>
      </div>
  ...
  bindRowClicks() {
    ...
    document.querySelectorAll('[data-action="go-detail"]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        App.navigate('detail', btn.dataset.id);
      });
    });
    ...
    document.querySelectorAll('[data-action="go-entry"]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        App.navigate('entry', btn.dataset.id);
      });
    });
    ...
    document.querySelectorAll('[data-action="open-status"]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        DashboardPage.openStatusModal(btn.dataset.id, btn.dataset.name);
      });
    });
  },
  ...
  openStatusModal(clientId, clientName) {
    ...
    const yearOpts = [now.getFullYear()-1, now.getFullYear(), now.getFullYear()+1].map(y =>
    ...
    document.getElementById('status-ai-btn').addEventListener('click', async () => {
      ...
      if (!apiKey) { App.toast('Нет DeepSeek ключа', 'error'); return; }
