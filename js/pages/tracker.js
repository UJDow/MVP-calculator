/* ============================================
   js/pages/tracker.js — Personal Time Tracker (ES Module)
   Portfolio BCHS v7.0
   Доступен только для: service_delivery, csm_analyst
...
export const TrackerAPI = {
  ...
  async getAll(force = false) {
    const now = Date.now();
    if (!force && this._cache && (now - this._cacheTs) < this.TTL) {
      return this._cache;
    }
    const res  = await fetch(`${BASE_URL}/tables/my_activities?limit=1000&sort=date`);
  ...
  async create(data) {
    const res = await fetch(`${BASE_URL}/tables/my_activities`, {
      method:  'POST',
  ...
  async update(id, data) {
    const res = await fetch(`${BASE_URL}/tables/my_activities/${id}`, {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify(data),
    });
  ...
  async delete(id) {
    await fetch(`${BASE_URL}/tables/my_activities/${id}`, { method: 'DELETE' });
    this._cache = null;
  },
...
export const TrackerPage = {
  ...
  async render(params = {}) {
    ...
    try {
      const [clientsRes, allActivities] = await Promise.all([
        fetch(`${BASE_URL}/tables/clients?limit=200&sort=name`).then(r => r.json()),
      ...
      const monthRows = allActivities.filter(r =>
  ...
  _buildHTML(todayRows, monthRows, allRows) {
    ...
    const clientOpts = this._clients.map(c =>
      `<option value="${c.id}"
        ${String(c.id) === String(this._filterClientId) ? 'selected' : ''}>
        ${c.name || c.company || c.id}
      </option>`
    ).join('');

    const typeOpts = Object.entries(this.TYPE_LABELS).map(([k, v]) =>
      `<option value="${k}">${v}</option>`
    ).join('');

    const durationOpts = this.DURATION_OPTIONS.map(m =>
      ...
      <div class="trk-page">
        ...
        <section class="trk-section" id="trk-today-section">
          <div class="trk-section-header">
            ...
            <span class="trk-section-badge">
              ${TrackerAPI.fmtDuration(todayMin)}
            </span>
          </div>
        ...
        <section class="trk-section" id="trk-month-section">
          <div class="trk-section-header">
            ...
            <span class="trk-section-badge">
              ${TrackerAPI.fmtDuration(monthMin)}
            </span>
          </div>
      ...
      <div class="trk-activity-row" data-id="${r.id}">
        ...
        <div class="trk-act-actions">
          <button class="trk-act-btn trk-act-edit"
                  data-id="${r.id}" title="Редактировать">✏️</button>
          <button class="trk-act-btn trk-act-del"
                  data-id="${r.id}" title="Удалить">🗑</button>
        </div>
      </div>`;
  },
  ...
  _renderMonthTable(rows) {
    ...
    for (const r of rows) {
      ...
      byClient[cid].types[r.type] =
        (byClient[cid].types[r.type] || 0) + r.duration_minutes;
    }

    const totalMin     = TrackerAPI.sumMinutes(rows);
    const totalBillMin = rows
      .filter(r => r.billable)
        ...
        <table class="trk-month-table">
          ...
          <tbody>
            ${sortedClients.map(([cid, data]) => {
              const pct      = totalMin > 0
                ? Math.round(data.minutes / totalMin * 100) : 0;
              const topType  = Object.entries(data.types)
                .sort((a, b) => b[1] - a[1])[0];
              const topLabel = topType
                ? (this.TYPE_LABELS[topType[0]] || topType[0]) : '—';
              const billMin  = data.rows
                .filter(r => r.billable)
                .reduce((s, r) => s + r.duration_minutes, 0);
              return `
                <tr>
                  <td class="trk-mt-client">
                    ${this._getClientName(cid)}
                  </td>
                  <td class="trk-mt-hours">
                    ${TrackerAPI.fmtHours(data.minutes)}ч
                  </td>
                  <td class="trk-mt-pct">
          ...
          <tfoot>
            <tr>
              ...
              <td>—</td>
              <td><strong>${TrackerAPI.fmtHours(totalBillMin)}ч</strong></td>
            </tr>
          </tfoot>
        </table>
      </div>`;
  ...
  _renderInsights(allRows, monthRows) {
    const monthMin  = TrackerAPI.sumMinutes(monthRows);
    const billMin   = monthRows
      .filter(r => r.billable)
      .reduce((s, r) => s + r.duration_minutes, 0);
    const billRatio = monthMin > 0
      ? Math.round(billMin / monthMin * 100) : 0;

    /* Топ клиент за всё время */
    const byClient = {};
    for (const r of allRows) {
      const cid = r.client_id || '__none__';
      byClient[cid] = (byClient[cid] || 0) + r.duration_minutes;
    }
    const topClientEntry = Object.entries(byClient)
      .sort((a, b) => b[1] - a[1])[0];
    const topClientName  = topClientEntry
      ? this._getClientName(topClientEntry[0]) : '—';
    const topClientHours = topClientEntry
      ? TrackerAPI.fmtHours(topClientEntry[1]) : '0';
    ...
    const topTypeLabel = topTypeEntry
      ? (this.TYPE_LABELS[topTypeEntry[0]] || topTypeEntry[0]) : '—';
    const topTypeHours = topTypeEntry
      ? TrackerAPI.fmtHours(topTypeEntry[1]) : '0';

    /* Средняя активность в день */
    const dayOfMonth = new Date().getDate();
    const avgPerDay  = dayOfMonth > 0
      ? (monthMin / dayOfMonth).toFixed(0) : 0;
    ...
    return `
      <div class="trk-insight-card">
        ...
        <div class="trk-insight-body">
          ...
          <div class="trk-insight-sub">
            ${TrackerAPI.fmtHours(billMin)}ч из
            ${TrackerAPI.fmtHours(monthMin)}ч
          </div>
        </div>
        <div class="trk-insight-bar-wrap">
          <div class="trk-insight-bar"
               style="--val:${billRatio}%; --color: var(--green)"></div>
        </div>
      </div>
      ...
      <div class="trk-insight-card">
        ...
        <div class="trk-insight-body">
          ...
          <div class="trk-insight-sub">
            ${topTypeHours}ч · ${avgPerDay}м/день в среднем
          </div>
        </div>
      </div>`;
  },
  ...
  /* ─── EVENTS ──────────────────────────────────────────────── */
  _bindEvents() {
    document.getElementById('trk-quick-form')
  ...
  async _handleSubmit(e) {
    ...
    try {
      ...
      if (this._editId) {
        await TrackerAPI.update(this._editId, payload);
        this._editId        = null;
  ...
  async _refreshMonthSection() {
    ...
    const existing = secEl?.querySelector('.trk-month-table-wrap, .trk-empty');
    if (existing) existing.outerHTML = this._renderMonthTable(rows);
  },
  ...
  async _openEditModal(id) {
    ...
    const dEl   = document.getElementById('trk-date');
    const cEl   = document.getElementById('trk-client');
    const tEl   = document.getElementById('trk-type');
    const durEl = document.getElementById('trk-duration');
    const nEl   = document.getElementById('trk-note');
    const bEl   = document.getElementById('trk-billable');
    const btn   = document.getElementById('trk-submit-btn');

    if (dEl)   dEl.value   = r.date       || '';
    if (cEl)   cEl.value   = r.client_id  || '';
    if (tEl)   tEl.value   = r.type       || 'call';
    if (nEl)   nEl.value   = r.note       || '';
    if (bEl)   bEl.checked = r.billable;
    if (btn)   btn.textContent = '💾 Сохранить';

    if (durEl) {
      const closest = this.DURATION_OPTIONS.reduce((prev, cur) =>
        Math.abs(cur - r.duration_minutes) < Math.abs(prev - r.duration_minutes)
          ? cur : prev
      );
      durEl.value = closest;
    }

    document.getElementById('trk-quick-section')
...
/* ── Регистрируем на window для совместимости ─────────────────
   app.js обращается к TrackerPage через модульный импорт,
   но внешний код может использовать window.TrackerAPI        */
