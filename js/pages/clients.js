  ...
  showForm(client) {
    ...
    window.App.openModal(`
      <div style="padding:4px 0 8px">
        ...
        <!-- Доступ к конечному клиенту — только для Partner -->
        <div id="cf-access-wrap" style="display:none;margin-bottom:10px">
    ...
    // ── показываем поле доступа только для Partner ──
    const typeSelect   = document.getElementById('cf-type');
    const accessWrap   = document.getElementById('cf-access-wrap');

    const toggleAccess = () => {
      accessWrap.style.display = typeSelect.value === 'Partner' ? 'block' : 'none';
    };

    typeSelect.addEventListener('change', toggleAccess);
    toggleAccess(); // сработает сразу при открытии (в т.ч. при редактировании)
